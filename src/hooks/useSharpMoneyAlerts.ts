...
export function useSharpMoneyAlerts(league: League, enabled: boolean = true) {
  ...
  useEffect(() => {
    if (!enabled || !user || !Array.isArray(trends) || trends.length === 0) return;
    
    trends.forEach((trend) => {
      // Check for reverse line movement (safely access nested property)
      if (trend?.lineMovement?.reverseLineMovement) {
        const signalKey = `rlm-${trend.matchId}`;
        if (!hasAlertedRecently(trend.matchId, signalKey)) {
          const advantageTeam = (trend.publicBetting?.spreadHome ?? 0) > 55 
            ? trend.awayTeam 
            : trend.homeTeam;
          
          createAlert(
            'reverse_line',
            `🔥 Reverse Line Movement: ${trend.homeTeam?.split(' ').pop() ?? 'Home'} vs ${trend.awayTeam?.split(' ').pop() ?? 'Away'}`,
            `Line moving against ${Math.max(trend.publicBetting?.spreadHome ?? 0, trend.publicBetting?.spreadAway ?? 0).toFixed(0)}% public action. Sharp money may be on ${advantageTeam?.split(' ').pop() ?? 'Unknown'}.`,
            trend.matchId,
            {
              homeTeam: trend.homeTeam,
              awayTeam: trend.awayTeam,
              publicHomePercent: trend.publicBetting?.spreadHome ?? 0,
              spreadMovement: trend.lineMovement?.spreadMovement ?? 0,
              league,
            }
          );
      ...
      // Check for strong sharp signals (safely access nested array)
      const signals = trend?.sharpBetting?.signals;
      const strongSignals = Array.isArray(signals) 
        ? signals.filter(s => s?.strength === 'strong') 
        : [];
      ...
      strongSignals.forEach((signal) => {
        ...
        if (!hasAlertedRecently(trend.matchId, signalKey)) {
          ...
          createAlert(
            ...
            `${signal.description} in ${trend.homeTeam?.split(' ').pop() ?? 'Home'} vs ${trend.awayTeam?.split(' ').pop() ?? 'Away'}. Confidence: ${trend.sharpBetting?.confidence ?? 0}%`,
            trend.matchId,
            {
              homeTeam: trend.homeTeam,
              awayTeam: trend.awayTeam,
              ...
              confidence: trend.sharpBetting?.confidence ?? 0,
              league,
            }
          );
  ...
  // Safe array for calculations
  const safeTrends = Array.isArray(trends) ? trends : [];
  
  return {
    isMonitoring: enabled && !!user,
    trendsCount: safeTrends.length,
    sharpSignalsCount: safeTrends.filter(t => {
      const signals = t?.sharpBetting?.signals;
      return Array.isArray(signals) && signals.length > 0;
    }).length,
    rlmCount: safeTrends.filter(t => t?.lineMovement?.reverseLineMovement === true).length,
  };
}
