...
export const fetchHeadToHeadHistory = async (
  ...
  try {
    ...
    return {
      ...
      streakTeam,
      streakCount,
      avgTeam1Score: lastMeetings.length > 0 ? Math.round((team1TotalScore / lastMeetings.length) * 10) / 10 : 0,
      avgTeam2Score: lastMeetings.length > 0 ? Math.round((team2TotalScore / lastMeetings.length) * 10) / 10 : 0,
